...
import { globalData, createBUser, getCTVAccountID, addMoney } from '../../fixtures';
...
test.describe('Pay CTV account', () => {
	...
	test('Pay CTV account for 1 to 12 months', async ({ page, browser }) => {
		for (let months = 1; months <= 12; months++) {
			await runCTVPaymentFlow(months, page, browser, loginPage, mainPage, paymentPage, thankYouPage, paymentPageURL, userID, ctvAccountID);
		}
	});
});
