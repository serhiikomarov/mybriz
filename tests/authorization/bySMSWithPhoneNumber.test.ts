...
test.describe("Authorization by SMS with phone number", () => {
  ...
  test("Authorization by SMS with invalid phone number UA", async () => {
    await authWithSMSPage.sendSMS(globalData.incorrectFormatPhoneNumber);
    await expect(authWithSMSPage.inputHelper).toContainText(errorMessages.ua.invalidPhoneNumber);
  });

  test("Authorization by SMS with invalid phone number EN", async ({ page }) => {
    await changeLanguage(page, setLanguage.en);
    await authWithSMSPage.sendSMS(globalData.incorrectFormatPhoneNumber);
